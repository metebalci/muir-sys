  ...
  LOOP  (SETQ I FIRST-SEQ)
	...
	(SELECTQ CH
	  ...
	  (#/D
	   (DESCRIBE I))
	  (#/E
	   (LET ((E (MA-INST-EXPANSION I)))
	     (DOLIST (W E)
	       (CADR:CC-TYPE-OUT (IF (NUMBERP W) W (CAR W)) CADR:CC-UINST-DESC T T)
	       (IF (LISTP W) (PRINT (CDR W))))))
	  (#/N
	   (SETQ PC (1+ PC)))
	  (#/O
	   (COND ((MA-INST-OP1 I)
	  ...
	  (#/S
	   ...
	   (SI:DESCRIBE-1 (MA-INST-AFTER-STATE I)))
	  (#/T
	   (LET ((TAG (MA-FIND-TAG (MA-INST-CODE I))))
	     (SETQ PC (MA-FIND-PC (GET TAG 'MA-TAG-POINTER) FIRST-SEQ)))))
	(GO LOOP)))

(DEFUN MA-FIND-TAG (CODELIST)
  (DOLIST (E CODELIST)
    (IF (AND (CONSP E)
	     (EQ (CAR E) 'UTAG))
	(RETURN (CADR E)))))

(DEFUN MA-FIND-PC (INST FIRST-INST)
  (PROG (C I)
	(SETQ C 0 I FIRST-INST)
     L  (COND ((NULL I) (FERROR NIL "CANT FIND IT"))
	      ((EQ INST I) (RETURN C)))
	(SETQ I (MA-INST-NEXT-INST I)
	      C (1+ C))
	(GO L)))
...
(DEFUN (CALL MA-DEST) (INST) 
  (PROG NIL (RETURN 'T (COND ((AND (CONSP (CAR (LAST INST)))
...
(DEFUN (MOVE MA-ASSEMBLE) (INST &AUX M A
  ...
  (SETQ CDR-CODING (IF (AND (CONSP (CADR INST))
