...
;;; Initializer for when machine has just been powered up.
...
(DEFUN CC-ZERO-ENTIRE-MACHINE (&OPTIONAL (RESET-P T))
  ...
  (IF RESET-P (DBG-RESET)) ;TEMPORARY KLUDGE?  JUST IN CASE MACHINE IS HUNG
  (LET ((CC-MODE-REG 40))	;Bind this because CC-RUN-TEST-LOOP looks at it!
    (IF RESET-P (CC-RESET-MACH))	;Disable all error halts, disable prom, slow speed
    ...
    (PROCESS-SLEEP 60. "Zero Machine")	;Let it run for 1 second, plenty long enough
  ...
  (IF RESET-P (CC-RESET-MACH)))		;Restore mode register and return
